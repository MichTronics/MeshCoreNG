...
class FemGainPrefs : public ConfigSerializer {
  protected:
    void structure() override {
        def("fem_rx_gain", fem_rx_gain);
    }

  public:
    uint8_t fem_rx_gain = 0;
};
...
TEST(NodePrefs, FemGainSettingsRoundTrip) {
    ...
    EXPECT_NE(std::string::npos, serialised.find("fem_rx_gain:0"));

    MockInputStream input(serialised.c_str());
    FemGainPrefs loaded;
    loaded.fem_rx_gain = 1;

    ASSERT_TRUE(loaded.loadSerial(input));
    EXPECT_EQ(0, loaded.fem_rx_gain);
}
